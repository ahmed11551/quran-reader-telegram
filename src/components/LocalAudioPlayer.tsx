...
export const LocalAudioPlayer: React.FC = () => {
  ...
  // Получаем URL для текущей суры
  const getAudioUrl = () => {
    const surahNum = currentSurah.toString().padStart(3, '0');
    const localFile = `/audio/surah_${surahNum}.mp3`;
    
    // Проверяем, есть ли локальный файл
    if (availableFiles.includes(`surah_${surahNum}.mp3`)) {
      return localFile;
    }
    
    // Если локального файла нет, используем внешние источники
    return `https://everyayah.com/data/Abdul_Basit_Murattal_192kbps/${surahNum}.mp3`;
  };
  ...
  const currentSurahNum = currentSurah.toString().padStart(3, '0');
  const hasLocalFile = availableFiles.includes(`surah_${currentSurahNum}.mp3`);
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex-1">
          ...
          <div className="flex items-center space-x-2 mt-2">
            ...
            {hasLocalFile && (
              <span className="text-xs text-green-600 font-medium">• Локальный файл</span>
            )}
          </div>
        </div>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-600">
            <strong>Файл:</strong> surah_{currentSurahNum}.mp3
          </div>
          <div className="flex items-center space-x-2">
            {hasLocalFile ? (
              <span className="text-green-600 text-sm font-medium">✅ Локальный</span>
            ) : (
              <span className="text-orange-600 text-sm font-medium">🌐 Внешний</span>
            )}
            <span className="text-xs text-gray-500">
              {availableFiles.length}/114 файлов
            </span>
          </div>
        </div>
      </div>
      ...
      {!hasLocalFile && (
        <div className="text-center">
          <button
            onClick={() => {
              const surahNum = currentSurah.toString().padStart(3, '0');
              const url = `https://everyayah.com/data/Abdul_Basit_Murattal_192kbps/${surahNum}.mp3`;
              window.open(url, '_blank');
            }}
            className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm font-medium flex items-center space-x-2 mx-auto"
          >
            <Download className="w-4 h-4" />
            <span>Скачать аудио файл</span>
          </button>
        </div>
      )}
